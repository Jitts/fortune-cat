...
export default function MonthlyOverview({ transactions }: { transactions: Transaction[] }) {
  ...
  return (
    <div className="rounded-2xl bg-white p-6 shadow-sm ring-1 ring-neutral-200">
      <h2 className="text-sm font-medium text-neutral-500">Monthly overview</h2>
      <ul className="mt-3 space-y-3">
        {months.map((m) => (
          <li key={m.key} className={m.key === currentKey ? "" : "opacity-80"}>
            <div className="flex items-baseline justify-between gap-3">
              ...
              <span
                className={`w-24 shrink-0 text-right text-sm font-semibold [font-variant-numeric:tabular-nums] ${
                  m.net >= 0 ? "text-emerald-700" : "text-neutral-900"
                }`}
              >
                {m.net >= 0 ? "+" : "−"}
            ...
            <div className="mt-0.5 flex justify-between pl-16 pr-24 font-mono text-[10px] text-neutral-400 [font-variant-numeric:tabular-nums]">
